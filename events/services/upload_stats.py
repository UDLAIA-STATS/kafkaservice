...
def handle_upload_stats(event: dict):
    ...
    try:
            ...
            try:
                with httpx.Client(timeout=30.0) as client:
                    ...
                    if player_response.status_code != 200:
                        logger.warning(f"No se encontró jugador para shirt_number {shirt_number}, marcando como None")
                        stat["player_id"] = None
                        processed_stats.append(stat)
                        continue
                    ...
                    if not player_id:
                        logger.warning(f"El jugador con shirt_number {shirt_number} no tiene ID, marcando como None")
                        stat["player_id"] = None
                        processed_stats.append(stat)
                        continue
                ...
                logger.exception(f"Error de red al procesar shirt_number {shirt_number}: {e}")
                stat["player_id"] = None
            except Exception as e:
                logger.exception(f"Error inesperado al procesar shirt_number {shirt_number}: {e}")
                stat["player_id"] = None
        ...
        logger.info(f"Enviando {len(processed_stats)} stats al endpoint: {stats_endpoint}")
        ...
        # logger.info(f"Stats subidas exitosamente para match_id {match_id}. Respuesta: {stats_response.status_code}")
